...
Pre-built, self-contained NanoVNA Firmware Updater executables (v1.0.2). Each binary
...
- **STM32 boards (NanoVNA-H / -H4 / gen1)** flash over USB DFU with `dfu-util`:
  - **Windows** — `dfu-util-static.exe` is fetched automatically (and is bundled in
    the `Firmware\` folder of the combined release zip). Install the ST DfuSe driver
    once via the `Driver\` folder at the repo root.
